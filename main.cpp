...
int main(int argc, char **argv)
{
  ...
  // llvm::StringRef inputFilename = "/home/nassimiheb/MLIR/llvm-project/mlir/test/Dialect/Linalg/tile-to-forall.mlir";
  ...
  root->setChildrenNodes(vectlist);
  std::cout << "Size " << vectlist.size() << std::endl;

  std::string RootEvel = evaluator.evaluateTransformation(root);

  root->setEvaluation(RootEvel);

  for (auto ChildNode : vectlist)
  {
    //   SmallVector<Node* , 2>   list1 = Tiling::createTilingCandidates(ChildNode, &context/*(ChildNode->getTransformedCodeIr())*/);
    //SmallVector<Node *, 2> list = Parallelization::createParallelizationCandidates(ChildNode, &context /*(ChildNode->getTransformedCodeIr())*/);

    // std::cout<<list1.size()<<std::endl;
    //ChildNode->setChildrenNodes(list);
  ...
  // std::string evel = evaluator.evaluateTransformation(list[4]);

  // list[4]->setEvaluation(evel);
