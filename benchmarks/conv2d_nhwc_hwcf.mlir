...
func.func @conv_2d_nhwc_hwcf(%arg0: !TTa, %arg1: !TTb , %arg2: !TTc) -> !TTc {
  %ret = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>,
                                     strides = dense<2> : tensor<2xi64>}
     ins (%arg0, %arg1: !TTa, !TTb )
    outs (%arg2: !TTc) -> !TTc
  return %ret : !TTc
}
...
func.func @main() {
  ...
  %c6 = arith.constant 112 : index
  %c8 = arith.constant 230 : index
  ...
  %in2D_tmp = call @alloc_4d_filled_f32(%c32, %c8, %c8, %c64, %val) : (index, index, index, index, f32) -> (tensor<?x?x?x?xf32>)
  %in2D_nhwc_temp = tensor.insert %f10 into %in2D_tmp[%c0, %c0, %c3, %c0] : tensor<?x?x?x?xf32>
  %in2D_nhwc = tensor.cast %in2D_nhwc_temp : tensor<?x?x?x?xf32> to !TTa
