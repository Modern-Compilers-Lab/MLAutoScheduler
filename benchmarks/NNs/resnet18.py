...
input_shape = (224, 224, 3)  # Typical input shape for ResNet
dummy_input = Input(shape=input_shape)

# Build the ResNet-18 model
output = resnet18(dummy_input, num_classes=1000)
...
dummy_data = np.random.random((1, 224, 224, 3)).astype(np.float32)
...
for i in range(20):
    ...
    # Make predictions on the dummy input
    output_data = run(dummy_data, model)
