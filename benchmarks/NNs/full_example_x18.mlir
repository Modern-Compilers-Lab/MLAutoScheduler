...
func.func @main(){
    ...
    %input = bufferization.alloc_tensor() : !tinput
    %filter = bufferization.alloc_tensor() : !tfilter
    %bias = bufferization.alloc_tensor() : !tbias
    %output = bufferization.alloc_tensor() : !toutput
    scf.for %i = %c0 to %n step %c1 {
      %t0 = func.call @nanoTime() : () -> (i64)
      %outputmain = func.call @conv(%input,%filter,%bias,%output) : (!tinput,!tfilter,!tbias,!toutput) -> (!toutput )
