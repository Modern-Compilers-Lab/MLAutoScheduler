...
!TTa =  tensor<1x2048x2048x32xf32>
!TTb =  tensor<3x3xf32>
!TTc =  tensor<1x2046x2046x32xf32>
func.func @conv() -> !TTc {
  ...
  %fake = linalg.fill ins(%val : f32) outs(%out1 : !TTb) -> !TTb
  %out2 = bufferization.alloc_tensor() : !TTc
  %fill = linalg.fill ins(%zero : f32) outs(%out2 : !TTc) -> !TTc
 ...
 %t0 = func.call @nanoTime() : () -> (i64)
  ...
  %dense_ret = linalg.pooling_nhwc_max {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>}
  ins(%input, %fake: !TTa, !TTb) 
  outs(%fill: !TTc) -> !TTc
